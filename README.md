...
集成自动拾取、一键装备处理、顺图、自动评分、技能全屏五大功能，自带游戏原生 XUI 配置面板（F11 呼出）、游戏内热键与公告输出。
...
| 技能全屏 | 遍历地图实体表，按 类型(529/273/545) ∧ 阵营(100/110/120/101/50) ∧ 血量>0 过滤目标，写参数槽调用技能CALL（`0x162C5E0`），全图怪物直接吃技能。参数（技能代码/伤害/频率/目标数）由 NF.ini 配置，热重载生效 | 热键 / 面板 |
| 原生 XUI 面板 | 游戏原生界面（非外部窗口），F11 呼出；显示配置状态、上次处理结果；可切换全部模式与规则 | F11 |
| 游戏内热键 | 只在游戏窗口内生效（钩子方式），不抢全局按键；8 个热键均可通过 NF.ini 修改 | — |
...
| `NFSkillFullscreen.h/.cpp` | 技能全屏：VirtualAlloc 4096 参数槽，遍历地图实体表过滤目标（类型+阵营+血量双白名单），写槽调技能CALL（ESI=槽 EAX=CALL）；参数来自 NF.ini `[技能全屏]`（热重载生效） |
...
| `[热键]` | 技能全屏 | `Alt+K` | 开/关技能全屏 |
| `[技能全屏]` | 技能代码 | `20044` | 技能CALL 使用的技能代码（需与服务器技能表匹配） |
| `[技能全屏]` | 伤害 | `99999` | 技能伤害 |
| `[技能全屏]` | 频率 | `500` | 轮询间隔(ms)，下限 50 |
| `[技能全屏]` | 目标数 | `8` | 每轮目标数量 1..20 |
...
- **控件约定**（ID 与 `NFNativeUi.cpp` 常量一一对应）：
  - 按钮：10=拾取开关、11=自动评分、12=GM模式开关、13=一键处理、14=技能全屏、20~23=顺图左/右/上/下。按钮文字静态，只收点击事件。
